...
XnStatus resolveModulesFile(XnChar* strFileName, XnUInt32 nBufSize)
{
    ...
    XnChar path[PATH_MAX];
    if (!CFURLGetFileSystemRepresentation(resourcesURL, TRUE, (UInt8*)path, PATH_MAX)) {
        fprintf(stderr, "failed to locate bundle path");
    }
    CFRelease(resourcesURL);
    nRetVal = xnOSStrCopy(strFileName, path, nBufSize);
    XN_IS_STATUS_OK(nRetVal);
    nRetVal = xnOSStrAppend(strFileName, "/modules.xml", nBufSize);
    //fprintf(stderr, "license path: %s\n", strFileName);
#else
    nRetVal = xnOSStrAppend(strFileName, "modules.xml", nBufSize);
#endif
	XN_IS_STATUS_OK(nRetVal);
...
XnStatus XnModuleLoader::LoadAllModules()
{
	...
	while (pModule != NULL)
	{
        ...
        nRetVal = xnOSStrAppend(path, "/", PATH_MAX);
        XN_IS_STATUS_OK(nRetVal);
        nRetVal = xnOSStrAppend(path, strConfigDir, PATH_MAX);
        XN_IS_STATUS_OK(nRetVal);
        //fprintf(stderr, "config path: %s\n", path);
		nRetVal = LoadModule(strModulePath, path);
#else
		const XnChar* strConfigDir = pModule->Attribute("configDir");
